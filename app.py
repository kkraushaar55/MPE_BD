# app.py — ATS-only Controls/Automation Jobs (Newest First)
...
def _clean_val(v) -> str:
    """Safely coerce CSV cell to string; treat NaN/None/NaT as empty."""
    try:
        s = str(v).strip()
...
with st.sidebar:
    ...
    st.caption("File: companies.csv · Columns: company,ats,token,api_base,industry")
        ...
        for _, row in wl.iterrows():
            ...
            try:
                if ats == "greenhouse" and tok:
                    ...
                    # Guard: Workday must return a list
                ...
                # silently skip malformed/empty rows
...
if not df.empty:
    ...
    if run:
        st.info("No jobs found. Check companies.csv (valid ATS + tokens), widen lookback days, then try again.")
    else:
